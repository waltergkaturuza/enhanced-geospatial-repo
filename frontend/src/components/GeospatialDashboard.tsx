...
type SidebarView = 'aoi' | 'boundaries' | 'imagery' | 'jobs' | 'downloads' | 'none';

const GeospatialDashboard: React.FC = () => {
  ...
  const handleBoundarySelect = (boundary: AdministrativeBoundary, selected: boolean) => {
    if (selected) {
  ...
  const handleBoundaryVisibilityChange = (boundaryId: number, visible: boolean) => {
  ...
  const handleMapViewChange = React.useCallback((viewState: MapViewState) => {
        ...
        <div className="w-96 bg-white border-r border-gray-200 flex flex-col">
          ...
          <div className="flex items-center justify-between p-2 border-b border-gray-200 bg-gray-50">
            <div className="flex items-center gap-1 flex-wrap">
              ...
              <button
                onClick={() => setSidebarView('imagery')}
                className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-medium transition-colors ${
                  sidebarView === 'imagery'
                    ? 'bg-blue-100 text-blue-700'
                    : 'text-gray-600 hover:bg-gray-100'
              ...
              <button
                onClick={() => setSidebarView('downloads')}
                className={`flex items-center gap-1 px-2 py-1 rounded text-xs font-medium transition-colors ${
